...
def load_audio_lstm_model() -> None:
    ...
    Loads weights from `audio_lstm_weights.pth` if the file exists;
    otherwise uses random initialisation (heuristic scoring still works).
    """
    ...
    if os.path.isfile(weights_path):
        try:
            ...
            print(f"[AudioLSTM] Loaded weights from {weights_path}")
        except Exception as e:
            print(f"[AudioLSTM] Could not load weights ({e}). Using random init.")
    else:
        print("[AudioLSTM] No pre-trained weights found — using random initialisation.")
