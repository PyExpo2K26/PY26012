...
def analyze_noise(image_path: str):
    ...
    try:
        ...
        # ── Resize to standard size to keep computation fast ───────────────
        h, w = img.shape[:2]
        max_dim = 512
        if max(h, w) > max_dim:
        ...
        combined_residual /= (3 * 3)  # average over channels & scales

        # ── Block variance map ─────────────────────────────────────────────
        block = max(16, min(h, w) // 16)
        var_map = _block_variance_map(combined_residual, block=block)

        # ── Score: coefficient of variation across blocks ──────────────────
        # Authentic images: relatively uniform noise → low CoV
        # Forged images:  high variance inconsistency → high CoV
        mean_var = np.mean(var_map) + 1e-9
        std_var  = np.std(var_map)
        coeff_of_variation = std_var / mean_var  # dimensionless

        # Calibration: CoV < 0.5 → natural; CoV > 2.0 → very suspicious
        score = float(np.clip((coeff_of_variation - 0.5) / 1.5, 0.0, 1.0))

        # ── Visualization ──────────────────────────────────────────────────
        # Up-sample var_map to full image size
        ...
        # Blend overlay with original for context
